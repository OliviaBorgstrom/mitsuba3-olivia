...
class VolumetricPathIntegrator2 : public MonteCarloIntegrator<Float, Spectrum> {
...
public:
    ...
    VolumetricPathIntegrator2(const Properties &props) : Base(props) {
    }
    ...
    struct retVals {
                ...
                Mask escaped_medium;
                Mask active_medium;
                Mask act_null_scatter;
                Mask act_medium_scatter;
                Mask active;
                Mask valid_ray;
                Mask specular_chain;
                Ray3f ray;
    ...
    auto medium_contribution(Ray3f ray, Sampler *sampler, UInt32 channel, MediumInteraction3f mei, 
                                    Mask needs_intersection, SurfaceInteraction3f si, Spectrum throughput,
                                     Mask active, UInt32 depth, Interaction3f last_scatter_event, MediumPtr medium, 
                                     const Scene *scene, Mask valid_ray, Mask specular_chain, Float last_scatter_direction_pdf,
                                     Mask active_medium, Mask active_surface, Mask act_null_scatter, Mask act_medium_scatter, Mask escaped_medium) const{
            ...
            if (dr::any_or<true>(act_medium_scatter)) {
                ...
                if (dr::any_or<true>(active_e)) {
                    ...
                    throughput *= phase_val * emitted * mis_weight(ds.pdf, dr::select(ds.delta, 0.f, phase_pdf));
                }
            ...
            return retVals {throughput,mei,si,needs_intersection,escaped_medium,
                            active_medium,act_null_scatter,act_medium_scatter,active,valid_ray, specular_chain, ray};
    }
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene,
        ...
        while (loop(active)) {
            ...
            out = medium_contribution(ray, sampler, channel, mei, 
                                    needs_intersection, si, throughput,
                                     active, depth, last_scatter_event, medium, 
                                     scene, valid_ray, specular_chain, last_scatter_direction_pdf,
                                     active_medium,active_surface, act_null_scatter, act_medium_scatter, escaped_medium);
            
            throughput = out.throughput;
            mei = out.mei;
            needs_intersection = out.needs_intersection;
            escaped_medium = out.escaped_medium;
            active_medium = out.active_medium;
            act_null_scatter = out.act_null_scatter;
            act_medium_scatter = out.act_medium_scatter;
            valid_ray = out.valid_ray;
            specular_chain = out.specular_chain;
            ray = out.ray;
            si = out.si;
            // throughput += thingy;
...
MI_IMPLEMENT_CLASS_VARIANT(VolumetricPathIntegrator2, MonteCarloIntegrator);
MI_EXPORT_PLUGIN(VolumetricPathIntegrator2, "Volumetric Path Tracer integrator");
